...
public class Generator {
    ...
    public static void main(String[] args) throws ParserConfigurationException, SAXException, IOException {
        ...
        deleteAutogeneratedFiles(corePackageName.asFile(outputDir));
        deleteAutogeneratedFiles(servicesPackageName.asFile(outputDir));
        deleteAutogeneratedFiles(modelPackageName.asFile(outputDir));
        deleteAutogeneratedFiles(clingimplServicesPackageName.asFile(clingImplOutputDir));
        deleteAutogeneratedFiles(clingimplServicesPackageName.asFile(testsOutputDir));
        ...
        configBuilder.add("ZoneGroupTopology", "ThirdPartyMediaServers", true);
        configBuilder.add("ZoneGroupTopology", "AvailableSoftwareUpdate", true);
        configBuilder.add("ZoneGroupTopology", "UpdateItem", true);
        configBuilder.add("ZoneGroupTopology", "ZoneGroupState", true);
        configBuilder.add("AVTransport", "LastChange", true);
        configBuilder.add("RenderingControl", "LastChange", true);
    ...
    private static HashCode calculateHash(String sourceCode) {
        ...
        return Hashing.md5().hashBytes(sb.toString().getBytes(UTF8));
    }
    ...
    private static void autogenerate(
        ...
        if (!typeFile.exists()) {
            ...
            System.out.println(Strings.padEnd(typeFile.getName(), 30, ' ') + " <= " + templateName );
        } else {
            System.out.println(Strings.padEnd(typeFile.getName(), 30, ' ') + " => SKIPPED" );
        }
    }

    private static void deleteAutogeneratedFiles(final File file) {
        if (!file.exists()) {
            return;
        }
        if (file.isDirectory()) {
            for (File f : file.listFiles()) {
                deleteAutogeneratedFiles(f);
            }
            ...
            try {
                final String firstLine = Files.readFirstLine(file, UTF8);
                if (firstLine.startsWith(AUTOGENERATED)) {
                    final String content = Files.toString(file, UTF8);
                    final HashCode hashCode = calculateHash(content);
                    final String oldHash = firstLine.substring(AUTOGENERATED.length());
                    final String newHash = hashCode.toString();
                    if (newHash.equals(oldHash)) {
                        file.delete();
                    } else {
                        System.err.println("WARNING: " + file.getName() + " was changed - will not touch it. Please remove the AUTOGENERATED comments. '" + oldHash + "' => '" + newHash + "'" );
                    }
                }
            } catch (IOException e) {
                e.printStackTrace();
