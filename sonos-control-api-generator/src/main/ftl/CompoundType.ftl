...
public final class ${data.javaClassName.name} implements Serializable {
    ...
    private final ${prop.javaClassName.name} _${prop.parameterName};
    </#list>

    private ${data.javaClassName.name}(
        <#list data.properties as prop>
        final ${prop.javaClassName.name} ${prop.parameterName}<#if prop_has_next>,</#if>
        </#list>
    ) {
        <#list data.properties as prop>
    ...
    <#list data.properties as prop>
    public ${prop.javaClassName.name} ${prop.getterName}() {
